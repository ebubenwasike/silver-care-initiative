...
# === SESSION TIMEOUT: Configure 10-minute session lifetime ===
    ...
    if any(key in session for key in ['loggedin', 'patient_loggedin']):
        ...
        if last_activity:
            try:
                ...
                # === SESSION TIMEOUT: Logout after 10 minutes inactivity ===
...
def session_check():
    ...
    if 'loggedin' in session:
        minutes_remaining = 10 - int((datetime.now() - datetime.fromisoformat(session.get('last_activity'))).total_seconds() / 60)
        return jsonify({
            'logged_in': True,
            'user_type': 'staff',
