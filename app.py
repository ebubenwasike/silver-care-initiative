...
# LOGIN PAGE for staff (missing earlier, restores endpoint 'login')
...
def login():
    if request.method == 'POST':
        ...
        valid_pw = stored_pw == password or (stored_pw and check_password_hash(stored_pw, password))
        if not valid_pw:
            flash('Incorrect email or password!', 'danger')
            return redirect(url_for('login'))
        # Stage for 2FA
...
# TEMPORARY ROUTE - GENERATE PASSWORD
@app.route('/generate_password')
def generate_password():
    password = "password123"  # change as needed
    hashed = generate_password_hash(password)
    return f"Hashed password for '{password}': <br><strong>{hashed}</strong>"

# SETUP 2FA (FIRST TIME OR SHOW QR)
...
def verify_2fa():
    ...
    # If it's POST (form submission), verify the code
    code = request.form.get('code', '').strip()  # must match name="code" in the form
    staff_id = session.get('pending_staff_id')

    print("🔍 2FA submitted code:", code)
    print("🔍 2FA staff_id from session:", staff_id)
    ...
    print("🔍 2FA secret from DB:", secret)

    totp = pyotp.TOTP(secret)
    current_valid = totp.now()
    print("🔍 Current TOTP on server:", current_valid)

    # You can allow a small time window:
    ...
    if is_valid:
        ...
        print("✅ 2FA verified, redirecting to staff dashboard")
        return redirect(url_for('staff'))
    else:
        flash('Invalid or expired 2FA code.', 'danger')
        return render_template('verify_2fa.html', email=staff['email'])
...
def staff():
    ...
    # Fetch patients created by this staff member (for table listing scope)
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
        """
    ...
    # Fetch total residents count (all patients in system)
    cursor_total = conn.cursor()
    cursor_total.execute("SELECT COUNT(*) FROM patients")
    total_patients_count = cursor_total.fetchone()[0]
    cursor_total.close()

    # Fetch staff profile details (name/email)
    cursor2 = conn.cursor(dictionary=True)
    cursor2.execute(
        "SELECT first_name, last_name, email FROM staff WHERE id=%s",
        (staff_id,)
    )
    staff_profile = cursor2.fetchone()
    cursor2.close()
    conn.close()

    # Render the improved UI template with staff data
...
# Return patients for the logged-in staff (JSON)
@app.route('/get_staff_patients')
def get_staff_patients():
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT id, first_name, last_name, email, phone, phn, emergency_contact, dob, created_at
            FROM patients
            WHERE created_by = %s
            ORDER BY created_at DESC
            """,
        ...
        # Coerce datetime to strings for JSON
        for p in patients:
            if 'created_at' in p and isinstance(p['created_at'], (datetime.date, datetime.datetime)):
        ...
        print('❌ Error fetching staff patients:', e)
        return jsonify(success=False, error=str(e)), 500

# All patients (for dashboard-wide view)
...
def get_all_patients():
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT id, first_name, last_name, email, phone, phn, emergency_contact, dob, created_at, created_by
            FROM patients
            ORDER BY created_at DESC
            """
        )
        ...
        def delete_patient(patient_id):
            ...
            try:
                conn = get_db_connection()
                cursor = conn.cursor()
                # Ensure patient belongs to this staff
                ...
                # Delete dependent rows first (appointments, vitals). If foreign keys with cascade exist this is still safe.
                ...
                # Attempt optional medications table if exists
                try:
        ...
        def patient_detail(patient_id):
            ...
            try:
                ...
                # Ownership check (only allow viewing if created_by matches or adjust if broader access needed)
                if patient.get('created_by') != session.get('id'):
                    cursor.close(); conn.close()
                    flash('Not authorized to view this patient.', 'danger')
                    return redirect(url_for('staff'))
                ...
                cursor.execute("""
                    ...
                    FROM vitals WHERE patient_id=%s ORDER BY updated_at DESC LIMIT 1
                """, (patient_id,))
                vitals = cursor.fetchone()

                # Medications placeholder attempt (table may not exist)
                medications = []
                try:
                    cursor.execute("SELECT name, dosage, schedule FROM patient_medications WHERE patient_id=%s", (patient_id,))
                ...
                if dob:
                    try:
                        if isinstance(dob, (datetime.date, datetime.datetime)):
                            ...
                            birth = datetime.datetime.strptime(str(dob), '%Y-%m-%d').date()
                        today = datetime.date.today()
                        age = today.year - birth.year - ((today.month, today.day) < (birth.month, birth.day))
                    except Exception:
                        age = None

                return render_template('patient_detail.html', patient=patient, vitals=vitals, medications=medications, age=age)
            except Exception as e:
                print('❌ Error loading patient detail:', e)
                flash('Error loading patient detail.', 'danger')
                return redirect(url_for('staff'))
        for p in patients:
            if 'created_at' in p and isinstance(p['created_at'], (datetime.date, datetime.datetime)):
                p['created_at'] = p['created_at'].strftime('%Y-%m-%d %H:%M')
