...
def login():
    if request.method == 'POST':
        ...
        valid_pw = stored_pw == password or check_password_hash(stored_pw, password)
        ...
        session['pending_first_name'] = account['first_name']
        session['pending_last_name'] = account['last_name']

        # If no secret → setup 2FA
        if not account['totp_secret']:
            return redirect(url_for('setup_2fa', staff_id=account['id']))

        # If secret exists → ask for code
