...
from validators import (
    ...
    validate_heart_rate, validate_respiratory_rate,
    validate_blood_pressure, validate_bmi, validate_weight, validate_height,
    validate_password, get_valid_phn,validate_temperature
)
# ------------------------------
...
db_config = {
    "host": "localhost",
    "user": "project275",              # MySQL username
    "password": "project275",  # MySQL password
...
def create_patient():
    ...
    if request.method == 'POST':
        ...
        # ------------------
        errors = []
        is_valid, msg = validate_name(first_name)
        if not is_valid:
            errors.append(msg)
            
        is_valid, msg = validate_name(last_name)
        if not is_valid:
            errors.append(msg)
...
def update_vitals():
    try:
        ...
        respiratory_rate = request.form.get('respiratory_rate', '').strip()
        ...
        if not blood_pressure:
            errors.append("Blood pressure is required (format: Systolic/Diastolic).")
        else:
            valid = validate_blood_pressure(blood_pressure)
            if not valid:
                errors.append("Blood pressure must be Systolic 80–200 / Diastolic 50–130.")

        valid = validate_age(age)
        if not valid:
            errors.append("Age must be between 40 and 60.")

        valid = validate_heart_rate(heart_rate)
        if not valid:
            errors.append("Heart rate must be between 40–180 bpm.")

        valid = validate_respiratory_rate(respiratory_rate)
        if not valid:
            errors.append("Respiratory rate must be between 10–30 breaths/min.")

        valid = validate_temperature(temperature)
        if not valid:
            errors.append("Temperature must be between 35.0–42.0 °C.")

        # Optional
        if bmi:
            valid = validate_bmi(bmi)
            if not valid:
                errors.append("BMI must be between 10 and 50.")
        ...
        if existing:
            cursor.execute("""
                ...
                SET blood_pressure=%s, bmi=%s, weight=%s, height=%s,
                    respiratory_rate=%s, temperature=%s, heart_rate=%s, age=%s, staff_id=%s
                WHERE patient_id=%s
            """, (blood_pressure, bmi, weight, height, respiratory_rate,
                  temperature, heart_rate, age, staff_id, patient_id))
        else:
            cursor.execute("""
                INSERT INTO vitals (patient_id, staff_id, blood_pressure, bmi, weight, height,
                                    respiratory_rate, temperature, heart_rate, age)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            """, (patient_id, staff_id, blood_pressure, bmi, weight, height,
                  respiratory_rate, temperature, heart_rate, age))
...
#GET VITALS (STAFF AND SENIORS) - KEEP ONLY ONE VERSION
...
def reset_password():
    ...
    if not validate_password(new_password):
        return jsonify({'success': False, 'message': 'Password too weak. Must be 8+ chars, include uppercase, lowercase, number, special char.'})
